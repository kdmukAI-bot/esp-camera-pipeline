...
/*
 * Limits derived from max QR version.
 * Under ESP-IDF, CONFIG_K_QUIRC_MAX_VERSION is set via Kconfig.
 * Standalone builds default to version 25.
 *
 * QR version N has (4*N+17) modules per side.
 * MAX_BITMAP = ceil(modules^2 / 8)
 * MAX_PAYLOAD is conservatively sized for the largest ECC-L numeric payload.
 */
#ifdef CONFIG_K_QUIRC_MAX_VERSION
#define K_QUIRC_MAX_VERSION_VAL CONFIG_K_QUIRC_MAX_VERSION
#else
#define K_QUIRC_MAX_VERSION_VAL 25
#endif

#define _K_QUIRC_MODULES(v) (4 * (v) + 17)
#define _K_QUIRC_BITMAP(v)  ((_K_QUIRC_MODULES(v) * _K_QUIRC_MODULES(v) + 7) / 8)

#define K_QUIRC_MAX_BITMAP  _K_QUIRC_BITMAP(K_QUIRC_MAX_VERSION_VAL)
#define K_QUIRC_MAX_PAYLOAD 2560 /* conservative upper bound */
